...
<h1>glTF JSON explorer</h1>
<div>
  Drop gltf and expand. There will be new extras keys for decoded binary accessor data.
</div>
`;
const JSONEditor = require ('jsoneditor');
//const SimpleDropzone = require ('simple-dropzone');
//const jsonDOM = document.getElementById("json-viewer");
const container = document.getElementById("jsoneditor");
const options = {'mode':'form';'modes':['form','tree','view','text']};
//console.log(JSONEditor);
let editor = new JSONEditor(container, options);
const inputEl = document.querySelector('#input');
const dropCtrl = new SimpleDropzone(container, inputEl);
